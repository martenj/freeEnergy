...
/**
 * this class parses a file for stacked basepairs for calculation of the free energy of RNA-Duplexes
 * 
 * @author marcel87
 * @version 1.0
 * 
 */
public class StackingParser extends EnergyParser {

	private double[] stack37 = new double[(int) Math.pow(N_BASES, N_BASES)];

	/*
	 * this way: 5' --> 3' w x y z 3' <-- 5'
	 * 
	 * !different to labeling in stack.dat!
	 */
	...
	public StackingParser() {
		super();
		// this("data/mfold/stack.dat");
	}
	 ...
	 *            path/to/file
	 */
	public StackingParser(String infile) {
	...
	protected boolean initialize() {
		...
		for (int i = 0; i < this.stack37.length; i++) {
			if (i < idx.length) {
				idx[i] = 0;
				maxIdx[i] = N_BASES;
			}
			this.stack37[i] = D_INF;
		}
								...
								if (m.find()) {
									...
									while (data.find()) {
										...
										if (!data.group().equals(".")) {
											...
											this.stack37[calcIdx(bases, maxIdx)] = Double.parseDouble(data.group());
										}
	...
	public double[] getStack37() {
		return stack37;
	}

	public double getStack37Value(int idx) {
		if ((idx >= 0) && (idx < stack37.length))
			return stack37[idx];
		else {
	...
	public double getStackedPairValue(String bases) {
		...
		if (bases.length() != 4) {
			...
			return stack37[calcIdx(b, maxIdx)];
		}
	}

	public void setStack37(double[] stack37) {
		this.stack37 = stack37;
	}
	 ...
	 */
	// @Override
	// public StackingParser setInfile(String infile) {
	 ...
	 */
	// @Override
	// public StackingParser setInfile(File infile) {
	// this.infile = infile;
	// return this;
	// }
